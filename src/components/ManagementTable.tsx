...
export function LoanManagementTable({
    ...
    //Modal Book
    const openBookModal = async (book: Book) => {
            ...
            case BookStatus.RESERVED:
                return (
                    <div className="px-3 py-1.5 rounded-lg bg-yellow-500/10 border border-yellow-500/30 flex items-center justify-center">
                        <span className="text-yellow-400 text-sm font-medium">Reservado</span>
                    </div>
                );
        }
    };

    const getBookStatusGradient = (book: Book) => {
        const status = book.status || BookStatus.AVAILABLE;
        switch (status) {
            case BookStatus.AVAILABLE:
                return "from-green-500/10 to-transparent";
    ...
    const getBookTitle = (loan: Loan) => {
        const bookId = loan.book?.id || loan.bookId || 0;
        return loan.book?.title || bookMap[bookId]?.title || `Libro #${bookId}`;
    };


    const getBookAuthor = (loan: Loan) => {
        const bookId = loan.book?.id || loan.bookId || 0;
        return loan.book?.author || bookMap[bookId]?.author || "—";
    };
    console.log('este es el selectedbook ', selectedBook);
    console.log('este es el hoveredLoan ', hoveredLoan);
                        ...
                        <div className="flex items-center justify-between mb-6">
                            <div className="flex items-center gap-4">
                                <div className="flex items-center gap-2">
                                    ...
                                    <h1 className="text-xl font-medium text-foreground">{isLoan ? "Historial de Préstamos" : 'Historial de Libros'}</h1>
                                </div>
                                        ...
                                        <div className="flex items-center gap-4">
                                            ...
                                            <div>
                                                <h3 className="text-lg font-bold text-foreground">
                                                    {getBookTitle(selectedLoan)}
                                                </h3>
                                                <span className="text-sm text-muted-foreground">
                                                    por {getBookAuthor(selectedLoan)}
                                                </span>
                                            </div>
                                        </div>
                                        ...
                                        <div className="bg-muted/40 rounded-lg p-3 border border-border/30">
                                            ...
                                            <div className="grid grid-cols-2 gap-3 mt-2">
                                                <div>
                                                    ...
                                                    <p className="text-sm font-medium">{getBookTitle(selectedLoan)}</p>
                                                </div>
                                                <div>
                                                    <span className="text-xs text-muted-foreground">Autor</span>
                                                    <p className="text-sm font-medium">{getBookAuthor(selectedLoan)}</p>
                                                </div>
                                                {(selectedLoan.book?.isbn || bookMap[selectedLoan.book?.id || selectedLoan.bookId || 0]?.isbn) && (
                                                    <div>
                                                        <span className="text-xs text-muted-foreground">ISBN</span>
                                                        <p className="text-sm font-mono font-medium">{selectedLoan.book?.isbn || bookMap[selectedLoan.book?.id || selectedLoan.bookId || 0]?.isbn}</p>
                                                    </div>
                                                )}
                                                {(selectedLoan.book?.publicationYear || bookMap[selectedLoan.book?.id || selectedLoan.bookId || 0]?.publicationYear) && (
                                                    <div>
                                                        <span className="text-xs text-muted-foreground">Año de Publicación</span>
                                                        <p className="text-sm font-medium">{selectedLoan.book?.publicationYear || bookMap[selectedLoan.book?.id || selectedLoan.bookId || 0]?.publicationYear}</p>
                                                    </div>
                                                )}
                                            </div>
                                        </div>
                    ...
                    <div className="relative border border-border/30 rounded-2xl p-6 bg-card">
                        ...
                        <div className="flex items-center justify-between mb-6">
                            <div className="flex items-center gap-4">
                                ...
                                <div className="text-sm text-muted-foreground">
                                    {books.filter(b => b.status === BookStatus.AVAILABLE).length} Disponibles • {books.filter(b => b.status === BookStatus.BORROWED).length} Prestados • {books.filter(b => b.status === BookStatus.RESERVED).length} Reservados
                                </div>
                            </div>
                        </div>
                            ...
                            {books.length === 0 ? (
                                ...
                                books.map((book, index) => (
                                    <motion.div
                                        ...
                                        className="relative cursor-pointer"
                                        onMouseEnter={() => setHoveredBook(book.id ?? index)}
                                    ...
                                    >
                                        ...
                                        >
                                            {/* Status gradient overlay */}
                                            <div
                                                className={`absolute inset-0 bg-gradient-to-l ${getBookStatusGradient(book)} pointer-events-none`}
                                                style={{
                                                    backgroundSize: "30% 100%",
                                                    backgroundPosition: "right",
                                                    backgroundRepeat: "no-repeat"
                        ...
                        {/* Book Detail Overlay - Inside Card */}
                        <AnimatePresence>
                            {selectedBook && (
                                <motion.div
                                    initial={{ opacity: 0 }}
                                ...
                                >
                                    ...
                                    <div className="relative bg-gradient-to-r from-muted/50 to-transparent p-4 border-b border-border/30 flex items-center justify-between">
                                        <div className="flex items-center gap-4">
                                            <div className="text-2xl font-bold text-muted-foreground">
                                                {String(books.indexOf(selectedBook) + 1).padStart(2, "0")}
                                            </div>
                                        ...
                                        <div className="flex items-center gap-2">
                                            ...
                                            {selectedBook.status !== BookStatus.BORROWED && (
                                                <motion.button
                                                    ...
                                                    onClick={() => selectedBook.id && deleteBookHandler?.(selectedBook.id)}
                                                    whileHover={{ scale: 1.02 }}
                                                    whileTap={{ scale: 0.98 }}
                                                >
                                                    <RotateCcw className="w-3 h-3" />
                                    ...
                                    <div className="flex-1 p-4 space-y-4 overflow-y-auto">
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                            ...
                                            <div className="bg-muted/40 rounded-lg p-3 border border-border/30">
                                                ...
                                                <div className="mt-1">
                                                    {getBookStatusBadge(selectedBook)}
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <div className="bg-muted/40 rounded-lg p-3 border border-border/30">
                                            ...
                                            <div className="grid grid-cols-2 gap-3 mt-2">
                                                <div>
                                                    <span className="text-xs text-muted-foreground">ISBN</span>
                                                    <p className="text-sm font-mono font-medium">{selectedBook.isbn}</p>
                                                </div>
                                                <div>
                                                    <span className="text-xs text-muted-foreground">Año de Publicación</span>
                                                    <p className="text-sm font-medium">{selectedBook.publicationYear}</p>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
